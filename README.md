...
```sh
# 从 npm(推荐,预构建产物直接可用)
npx @deepseek-ai/dsh plugin --profile web add irmia-devkit

# 从 GitHub(锁定版本)
npx @deepseek-ai/dsh plugin --profile web add github:irmia2026/irmia-devkit#v0.1.0
```

安装后可先验证配置层再启动:

```sh
npx @deepseek-ai/dsh --profile web --dump-config   # 应出现 "# == irmia-devkit" 层
